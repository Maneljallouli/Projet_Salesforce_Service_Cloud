...
// AJOUT : Import de la méthode Apex
...
export default class LuxHeader extends NavigationMixin(LightningElement) {
    ...
    // Nouveaux tracks pour les notifications
    @track notifDropdownOpen = false;
    @track hasNotifications = false; 
    @track notifications = []; // Initialisation vide pour que la boule ne s'affiche pas

    // AJOUT : Récupération dynamique des notifications
    ...
    toggleNotifDropdown() { 
        ...
        if(this.notifDropdownOpen) this.hasNotifications = false; // Cache la boule rouge à l'ouverture
    }
