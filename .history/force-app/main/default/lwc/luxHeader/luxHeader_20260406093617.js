...
export default class LuxHeader extends NavigationMixin(LightningElement) {
    ...
    _wiredNotifResult;
    _subscription  = null;
    _hasBeenOpened = false; // flag : le dropdown a été ouvert au moins une fois
    ...
    toggleNotifDropdown() {
        if (this.notifDropdownOpen) {
            // ── FERMETURE : on marque comme lues maintenant ──
            this._closeAndMarkRead();
        } else {
            // ── OUVERTURE : on affiche juste, on ne touche à rien ──
            this.notifDropdownOpen = true;
            this.hasUnread         = false; // badge disparaît visuellement
    ...
    _closeAndMarkRead() {
        ...
        if (this._hasBeenOpened && this.notifications.length > 0) {
            markAllAsRead()
                .then(() => {
    ...
    closeNotifDropdown() {
        // Appelé par l'overlay — même comportement que fermeture
        this._closeAndMarkRead();
    }
