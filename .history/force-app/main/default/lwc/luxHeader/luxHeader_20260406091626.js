...
export default class LuxHeader extends NavigationMixin(LightningElement) {
    ...
    // ── badge non-lu (disparaît après ouverture) ──
    @track hasUnread = false;

    // ── liste complète (ne disparaît jamais tant qu'il y a des données) ──
    @track notifications = [];
    ...
    wiredNotifs(result) {
        ...
        if (data) {
            ...
            // Badge non-lu seulement si nouvelles données arrivent
            if (data.length > 0) {
                this.hasUnread = true;
            }
        } else if (error) {
    ...
    toggleNotifDropdown() {
        ...
        // ── On efface uniquement le badge non-lu, PAS la liste ──
